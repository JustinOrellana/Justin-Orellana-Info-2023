...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("school_name", "Choose a School", choices = songs_df$school),
      htmlOutput(outputId = "song_name"),
      htmlOutput(outputId = "composer"),
      htmlOutput(outputId = "lyrics"),
      br()
...
server <- function(input, output) {
  ...
  output$song_name <- renderUI({
    ...
    get_song_name(songs_df, input$school_name)
  ...
  output$composer <- renderUI({
    ...
    get_info(songs_df, input$school_name)
  ...
  output$lyrics <- renderUI({
    ...
    get_lyrics(songs_df, input$school_name)
  ...
  output$scatter <- renderPlotly({
    ...
    p <- ggplot(songs_df, aes(x = sec_duration , y = bpm, text = school)) +
      geom_point(aes(color = songs_df$school)) +
      geom_text(aes(label = ifelse(school == input$school_name, school, ""))) +
      labs(color = "school") 
  ...
  output$songs <- renderUI({
    ...
    # this dataframe.
    filtered_df <- get_relevant_songs(input$word)
